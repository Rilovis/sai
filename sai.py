...
helpMessage =""" Š€£Բ ฿✪Ŧ β¥.Šαї

Tagall ➠แท็ก500คนลง
@@@ ➠แท็ก500คนลง
เช็คแอด ➠เช็คแอดห้ออง
...
Com:off ➠เปิดคอมเม้น
Glist ➠เช็ครายการกลุ่ม
Cancel ➠ยกเลิกค้างเชิญ
Mid @ ➠เช็คMid
Invite ➠เชิญ คอนแทค
Invite: ➠เชิญ+Mid
Whitelist @ ➠บัญชีขาว
Whitelist: ➠บัญชีขาว+Mid
Whitelist ➠บัญชีขาว+คอนแทค
Blacklist @ ➠บัญชีดำ
Blacklist: ➠บัญชีดำ+Mid
Blacklist ➠บัญชีดำ+คอนแทค
Clear ban ➠ลบรายการแบล็คลิสทั้งหมด
Link on ➠เปิดลิ้งค์
Link off ➠ปิดลิ้งค์
Gurl ➠เปิดลิ้งค์กลุ่ม
Url ➠รับลิงค์
Gname: ➠เปลี่ยนชื่อกลุ่ม
Banlist ➠ตรวจสอบรายการดำ
Details grup ➠รายละเอียดกลุ่ม+Gidกลุ่ม
Inviteme: ➠เชิญ+Gidกลุ่ม
Info grup ➠เช็ครายละเอียดกลุ่ม
Clear grup ➠ล้างกลุ่ม
Nuke ➠ล้างกลุ่ม
Ratakan ➠ล้างกลุ่ม
Kick @ ➠เตะ
Kick: ➠เตะ+mid
...
auto cancel on ➠ยกเลิกรัน
set. ➠เช็คตั้งค่า
Message set: ➠เพิ่มข้อความ
Pesan add: ➠เพิ่มข้อความ
Pesan add check ➠เช็คข้อความ
Change ➠เช็คข้อความ
Message set: ➠เพิ่มข้อความ
Comment set: ➠เพิ่มข้อความ
Com ➠เช็คข้อความ
Add staff @ ➠เพิ่มแอด
Remove staff @ ➠ลบแอด
Stafflist ➠เช็ครายการแอด
Kick ➠เตะ+ชื่อ
List grup ➠เช็คกลุ่มที่อยู่
Cancel invite: ➠ยกเลิกกลุ่มเชิญ
Accept invite ➠ยอมรับคำเชิญกลุ่มทั้งหมด
Mysteal @ ➠ขโมย
Mybackup ➠การสำรองข้อมูลของฉัน
Backup ➠สำรองข้อมูลของฉัน
.music ➠เพลง
.Youtube ➠ลิ้งค์ยูทูป
blocklist ➠รายการบล้อค
kill ➠เตะรายการดำ
Ban cek ➠เช็ค
Good bye/bye ➠ออกจากกลุ่ม
Cancelall ➠ยกเลิกห้องที่ค้างเชิญ
...
def bot(op):
    try:
        ...
        if op.type == 25:
            ...
            if msg.contentType == 13:
            ...
            elif msg.text in ["Bot1 Gift","Bot1"]:
                msg.contentType = 9
            ...
            elif msg.text in ["Bot2 Gift","Bot2"]:
                msg.contentType = 9
                msg.contentMetadata={'PRDID': '3b92ccf5-54d3-4765-848f-c9ffdc1da020',
                                    'PRDTYPE': 'THEME',
                                    'MSGTPL': '3'}
            ...
            elif msg.text in ["Bot3 Gift","Bot3"]:
                msg.contentType = 9
                msg.contentMetadata={'PRDID': '3b92ccf5-54d3-4765-848f-c9ffdc1da020',
                                    'PRDTYPE': 'THEME',
                                    'MSGTPL': '4'}
                msg.text = None
                ki3.sendMessage(msg)
            elif msg.text in ["Bot4 Gift","Bot4"]:
                msg.contentType = 9
                msg.contentMetadata={'PRDID': '3b92ccf5-54d3-4765-848f-c9ffdc1da020',
                                    'PRDTYPE': 'THEME',
                                    'MSGTPL': '5'}
...
#--------------------------------------------------------
              ...
              if msg.from_ in admin:
                ...
                cl.sendText(msg.to,"." + str(wait["comment"]))
...
#--------------------------CEK SIDER เช็คระบบอ่าน------------------------------
            elif msg.text == "เช็ค":
                if msg.toType == 2:
                    cl.sendText(msg.to, datetime.now().strftime('%Y/%m/%d %H:%M:%S'))
                    try:
...
#--------------------------------------------------------
                ...
                if msg.toType == 2:
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "รายชื่อคนที่อ่าน\n\n╔══════════%s\n╚══════════\n\nรายชื่อคนที่อ่านล่าสุด\n\n╔══════════\n%s╚══════════\n\nวันที&เวลา\n[%s]\n " % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                        print "ReadPoint Set..."
                        try:
                            del wait2['readPoint'][msg.to]
                            del wait2['readMember'][msg.to]
                        ...
                        wait2['ROM'][msg.to] = {}
                        print wait
                        cl.sendText(msg.to, + datetime.now().strftime('\n%Y-%m-%d %H:%M:%S'))
                    else:
                        cl.sendText(msg.to, "ระบบไม่สามารถตรวจจับรายชื่อได้ สั่ง เช็ค แล้วสั่งคำว่า อ่าน ใหม่อีกรอบ")
...
#--------------------- เช็คแอดห้อง -----------------------------------
    ...
    #-------------Fungsi Tag All Start---------------#
            elif msg.text in ["SAI","sai","ทรายแท็ก","ทรายแท็กชื่อ","ฺBot.","แ���็ก"]:
                  group = cl.getGroup(msg.to)
...
#-----------------------------------------------
                        ...
                        ki5.updateGroup(G)
#-----------------------------------------------
            elif msg.text in ["bye","Dadah","Good bye","Sayonara"]:
                if msg.toType == 2:
                    ginfo = cl.getGroup(msg.to)
                    try:
